...
sudo setfacl -R -m u::rwx,g::rwx,g:dls_dasc:rwx,g:gda:rwx,o::rx,d:u::rwx,d:g::rwx,d:g:dls_dasc:rwx,d:g:gda:rwx,d:o::rx /dls/${BEAMLINE}/software/gda

echo "beamline staff edit perms to config..."
sudo setfacl -R -m g:${BEAMLINE}_staff:rwx,d:g:${BEAMLINE}_staff:rwx /dls/${BEAMLINE}/software/gda/config/

echo "beamline staff edit perms to beamline bin..."
sudo setfacl -R -m g:${BEAMLINE}_staff:rwx,d:g:${BEAMLINE}_staff:rwx /dls/${BEAMLINE}/bin/

echo "everyone permissions to config/var and beamline var..."
sudo setfacl -R -m o::rwx,d:o::rwx /dls/${BEAMLINE}/software/gda/config/var
sudo setfacl -R -m o::rwx,d:o::rwx /dls/${BEAMLINE}/var

echo "everyone permissions write to command history file ..."
sudo setfacl -R -m o::rwx,d:o::rwx /dls/${BEAMLINE}/scripts/.cmdHistory.txt

echo "everyone permissions write to .metadata (shared workspace, deprecated in next version 8.0)..."
sudo setfacl -R -m o::rwx,d:o::rwx /dls/${BEAMLINE}/.metadata

echo "GDA remote Server ${BEAMLINE}ssh.key must by owned by gda"
sudo chown gda:dls_dasc /dls/${BEAMLINE}/software/gda/config/${BEAMLINE}ssh.key
sudo chown gda:dls_dasc /dls/$BEAMLINE/${BEAMLINE}ssh.key
